 ...
 */
function preprocessText(text: string, indexer: number[]): string {
  // Normalize text
 ...
 */
function processText(textList: string[], indexer: number[]): { textIds: number[][]; textMask: number[][][] } {
  const processedTexts = textList.map(t => preprocessText(t, indexer));
...
async function synthesize(
  ...
  for (const chunk of textChunks) {
    ...
    const { textIds, textMask } = processText([chunk], textProcessor);
    const bsz = 1;
